...
utadm command need some updates
...
If you install new Hipster it is necessary to have the GNOME packages before obsoleting
...
If after logout the gdm-login not reappears, helps the following script installed at /opt/SUNWut/lib/gdm/revivesrsession.py and called by the helper script /etc/opt/SUNWut/gdm/SunRayPostSession/helpers/revivesession
    ...
    dpl = [ p.split()[2].strip(':')	for p in sp.Popen(['ptree', pid], stdout=sp.PIPE).stdout.readlines() if '/opt/SUNWut/lib/Xnewt' in p ]
